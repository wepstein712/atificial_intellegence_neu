...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        smallestGDist = 1000000
        closestGPos = None
        bestAction = None
        for action in legal:
            pacPos = successorPosition = Actions.getSuccessor(pacmanPosition, action)
            for pos in ghostPositions:
                dist = self.distancer.getDistance(pacPos, pos)
                if smallestGDist > dist or not closestGPos or not bestAction:
                    smallestGDist = dist
                    closestGPos = pos
                    bestAction = action
